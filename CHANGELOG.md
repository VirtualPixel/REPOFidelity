...
- F7 Individual-Opt Probe (new diagnostic): cycles through 17 runtime-toggleable optimizations, samples the same scene with each one forced OFF for 5s, restores, computes per-opt frame-time delta vs baseline. Output is a clipboard-ready table with Δ avg ms / Δ 1% low / Δ worst frame for each optimization. On-screen progress bar (orange) and status text via the existing overlay. Built for gathering numbers to present to the studio with the "should this go in the base game?" pitch. ~140s total runtime on the full sweep. Coverage: CPU patches bundle, GC NonAlloc bundle, allocation fixes, GPU instancing, particle culling mode, zero-intensity light shadows, range-tiered shadow res, player avatar shadow + bone-update skip, small-light shadow budget, plus all 7 PerfXxx-gated scene mutations. F8 picked up the old `LightDiagnostics.Run()` binding that F7 used to own
...
- F9 Cost Probe: A/B sweep added (temporary diagnostic). Runs four 8s baseline samples in ON-OFF-ON-OFF order on the same procedural map, reports per-sample numbers + ON/OFF averages + a cache-warmup bias check (ON#2 vs ON#1) so a measured delta can be told apart from second-window-faster cache effects. Adds ~24s to probe runtime
