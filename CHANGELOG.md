...
- PhysGrabObjectGrabArea.Update was calling `playerGrabbers.ToList()` every frame to support safe iteration. When no one's grabbing the object, this still allocates an empty List per instance per frame (~660/sec across a typical scene). Added a fast-path that skips the entire Update when both `playerGrabbing` and `listOfAllGrabbers` are empty — the original cleanup loops have nothing to do in that state anyway. Confirmed: per-call cost dropped from ~0.62 µs to ~0.21 µs in 5090/9950x A/B testing
- AudioListenerFollow.Update was rebuilding `LayerMask.GetMask(new string[] { "LowPassTrigger" })` and allocating a Collider[] from OverlapSphere on every 15Hz tick. Single instance so individually small, but a constant gen0 drip across the whole session. Layer mask cached statically, OverlapSphere swapped to NonAlloc
- Both patches are gated on `ModEnabled && AllocationFixesEnabled`. F10 disables them via ModEnabled. The independent flag exists so the F9 probe can flip just these patches for the A/B comparison without touching SceneOptimizer state
- F9 Cost Probe: A/B sweep added. Runs four 8s baseline samples back-to-back in ON-OFF-ON-OFF order, on the same procedural map. Reports per-sample numbers, ON/OFF averages, and an order-bias check (ON#2 vs ON#1) so we can tell whether a measured delta is patches helping vs second-window-faster cache warmup. Adds ~24s to probe runtime
- AudioLowPassLogic.CheckLogic full method replacement was tried and reverted before shipping. Even though it removed real allocations (Collider[] + RaycastHit[] + List per call), Harmony Prefix dispatch overhead on a 4Hz × 170-instance call site exceeded the savings on fast CPUs (5090/9950x A/B showed -0.14 ms with the patch active). NonAlloc-via-Harmony is only a net win when the original method is expensive enough to amortize the dispatch — CheckLogic isn't
