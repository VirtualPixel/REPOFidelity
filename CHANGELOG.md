...
- F7 Individual-Opt Probe (new diagnostic): cycles through every runtime-toggleable optimization, samples the same scene with each one forced OFF for 5s, restores, computes per-opt frame-time delta vs baseline. Output is a clipboard-ready table with Δ avg ms / Δ 1% low / Δ worst frame for each optimization. Built for gathering numbers to present to the studio with the "should this go in the base game?" pitch. ~85s total runtime. F8 picked up the old `LightDiagnostics.Run()` binding that F7 used to own
