...
- Two fixes on the pause-menu avatar preview, the last thing the 1.7.7 crash report logs before the game dies. The mod bumps that preview's render texture so the edges aren't jagged, and the target was 1024, applied to the short edge. The preview is 208x416, so with the aspect preserved what actually landed was 1024x2048: four times the surface the setting asks for, on a UI slot about 400px tall, thrown away and reallocated on every menu open. It targets the long edge now, so the same preview gets 512x1024 and looks the same. Nothing the mod adds to that preview used to come off until the next menu open either, so the post-process layer and the render gate stayed on a camera whose texture the game had already released, and the mod kept a reference to both for the rest of the run. All of it is handed back the moment the game tears the preview down. Whether that is the whole crash is still open; the report is at issue #14 and ChaseFarley2 sent a very good log.
- Particle and light shadows go back to what the prefab shipped instead of being forced on. When the preset was not optimizing, the particle pass flipped every non-casting particle renderer to casting: 907 systems on Museum, 1316 on Wizard, none of which vanilla ever shadows, and nothing recorded the starting state so F10 could not put them back. Item lights, explosion lights and the glow-light budget stamped Soft shadows the same way. All four record what the prefab had, cut when the preset asks, and restore exactly that otherwise. Mid-level spawns share the same record, so a light that appears after level gen is restorable too.
- One scene sweep per frame instead of one per joining player. The optimization pass walks the whole scene, ten object scans plus a watchlist that hit 5250 renderers on Museum, and it ran once for every PlayerAvatar that started. A lobby fills in a single frame, so a full group meant that sweep six times back to back for one result.
- Opening Settings then Language while the mod's graphics page was up used to break F10 for the rest of the session. Every other button on that row closed the mod page properly; Language did not, so the page was gone but the mod still thought it was open, and it gates the F10 vanilla-compare on exactly that. A status ticker also kept running against the closed page.
