...
- Ultrawide: a field probe for the upgrades-stay-on-screen report. 1.7.5's parked-cull rework didn't clear it, and every log so far was missing the element entirely because the reveal census logs each graphic only once and only if it happens to be scanned mid-reveal. There's now a short diagnostic burst (a `[ultrawide] leak-probe:` line) that, while the wide presentation is active, names every HUD graphic actually sitting in the revealed band that isn't already hidden, and tags why the cull skipped it (owner parked, still shown, hides in place, or no owner). It's log-only, no behavior change; it exists so the next report from the affected panel pins the real cause instead of another guess.
