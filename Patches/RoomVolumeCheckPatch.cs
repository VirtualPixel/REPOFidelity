...
static class RoomVolumeCheckPatch
{
    ...
    static void ApplyGameLogicTail(RoomVolumeCheck instance, PlayerAvatar player)
    {
        if (player && instance.CurrentRooms.Count > 0)
        {
            ...
            if (sameMap)
            {
                // v0.4.0-tester: SetExplored returns bool (was void). Local player earns
                // scout points scaled by distance-to-truck on first discovery of each room.
                bool explored = instance.CurrentRooms[0].SetExplored();
                if (explored && player.isLocal
                    && TruckSafetySpawnPoint.instance
